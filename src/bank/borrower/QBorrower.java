...
public class QBorrower {
	...
	private String responseq = "queue.10253875";
	...
	public void startTopicConnection(Context ctx) {
		...
		try {
			...
			pubSession = tConnection.createTopicSession(false,
					Session.AUTO_ACKNOWLEDGE);

			publisher = pubSession.createPublisher((Topic) ctx
					.lookup(loanTopic));
	...
	private void sendLoanRequest(String borrower, double salary, double loanAmt, int paymentScheme) {
		try {
			...
			topicmsg.setJMSExpiration(new Date().getTime() + 30000); // Expires
																		// in 30
																		// seconds
...
//			 Wait to see if the loan request was accepted or declined
			...
			ObjectMessage objMsg = (ObjectMessage) qReceiver.receive(30000);
			LoanProposalData lpd =  (LoanProposalData) objMsg.getObjectProperty("LoanProposal");
			System.out.println(lpd.toString());
//			 The borrower then chooses the best offer and sends the ACCEPT
//			 text message back to the lender.
//			 The JMSCorrelationID of the ACCEPT message is set to the lender’s
//			 loan proposal message ID.
	...
	public static void main(String argv[]) {
		...
		try {
			...
			System.out
					.println("Enter: Full Name, Salary, Loan_Amount, Loan Term(in terms)");
